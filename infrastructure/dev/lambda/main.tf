#------------------------------------------------------------------------------
# Dev Environment Lambda - Release Metadata API
#------------------------------------------------------------------------------
...
resource "aws_lambda_function" "release_metadata" {
  ...
  # Artifact from S3 bucket in Tooling account
  s3_bucket = var.artifacts_bucket_name
  s3_key    = var.lambda_artifact_key

  # Environment variables for release metadata
  environment {
    variables = {
      ENVIRONMENT     = var.environment
  ...
  lifecycle {
    # Ignore changes to deployed_at timestamp
